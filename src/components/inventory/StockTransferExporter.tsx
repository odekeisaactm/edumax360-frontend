...
function buildPrintHTML(rows: ExportRow[], title: string, schoolName?: string): string {
  ...
  const headers = ['#', 'Receipt No.', 'From Location', 'To Location', 'Items Moved', 'Date'];

  const bodyRows = rows.map((r, i) => {
    const cells = [
      i + 1,
      `<span style="font-family:monospace; font-weight:bold;">${r.receiptNumber}</span>`,
      r.fromLocation,
      r.toLocation,
      `<strong>${r.itemCount}</strong>`,
      r.transferDate,
    ];
    return `<tr class="${i % 2 === 0 ? 'even' : ''}">${cells.map(c => `<td>${c}</td>`).join('')}</tr>`;
  }).join('');

  const totalItemsMoved = rows.reduce((sum, r) => sum + r.itemCount, 0);
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .header { text-align: center; margin-bottom: 18px; border-bottom: 2px solid #8b5cf6; padding-bottom: 14px; }
    .header .school { font-size: 13px; font-weight: 700; color: #8b5cf6; margin-bottom: 4px; }
    ...
    th { background: #8b5cf6; color: #fff; padding: 7px 8px; text-align: left; font-size: 9.5px; font-weight: 700; text-transform: uppercase; }
    td { padding: 6px 8px; border-bottom: 1px solid #f1f5f9; vertical-align: middle; }
    tr.even td { background: #f8fafc; }
    .totals { margin-top: 14px; text-align: right; font-size: 11px; }
    .totals strong { color: #8b5cf6; font-size: 13px; }
...
<body>
  <div class="header">
    ...
    <h1>${title}</h1>
    <div class="meta">Generated: ${now} &nbsp;|&nbsp; Total Batches: ${rows.length}</div>
  </div>
  <table>
    <thead><tr>${headers.map(h => `<th>${h}</th>`).join('')}</tr></thead>
    <tbody>${bodyRows}</tbody>
  </table>
  <div class="totals">Total Items Moved (Across Batches): <strong>${totalItemsMoved.toLocaleString()}</strong></div>
  <script>window.onload = function() { window.print(); };</script>
</body>
</html>`;
}

export default function StockTransferExporter({ schoolName, getExportRows }: Props) {
  ...
  const resolvedTitle = title.trim() || `Stock Transfer Ledger – ${new Date().toLocaleDateString('en-NG')}`;
  ...
  const handleExcel = async () => {
    ...
    try {
      ...
      const sheetRows = rows.map((r) => ({
        ...
        'Line Items': r.itemCount,
        'Transfer Date': r.transferDate,
      }));

      const ws = XLSX.utils.json_to_sheet(sheetRows);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Transfer Ledger');
      XLSX.writeFile(wb, `stock_transfer_ledger_${new Date().toISOString().slice(0, 10)}.xlsx`);
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                <Download className="h-5 w-5 text-violet-600" /> Export Stock Transfers
              </h3>
            ...
            <div className="p-6 space-y-5">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 text-sm border border-slate-200 rounded-xl focus:ring-2 focus:ring-violet-500 outline-none"
                />
              </div>
              ...
              <div className="flex items-center gap-3 justify-end pt-2 border-t border-slate-100">
                ...
                <button onClick={handleExcel} disabled={exporting} className="inline-flex items-center gap-2 px-5 py-2.5 bg-violet-600 text-white text-sm font-semibold rounded-xl hover:bg-violet-700 disabled:opacity-50">
                  {exporting ? <Loader2 className="h-4 w-4 animate-spin" /> : <FileSpreadsheet className="h-4 w-4" />} Excel
                </button>
                <button onClick={handlePDF} disabled={exporting} className="inline-flex items-center gap-2 px-5 py-2.5 bg-violet-600 text-white text-sm font-semibold rounded-xl hover:bg-violet-700 disabled:opacity-50">
