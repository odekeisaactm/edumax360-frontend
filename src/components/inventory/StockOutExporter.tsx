...
function buildPrintHTML(rows: ExportRow[], title: string, schoolName?: string): string {
  const now = new Date().toLocaleString('en-NG', { dateStyle: 'long', timeStyle: 'short' });
  const headers = ['#', 'Item', 'Qty Removed', 'Reason', 'Location', 'Staff/Dept', 'Date', 'Notes'];

  const bodyRows = rows.map((r, i) => {
    const cells = [
      i + 1,
      `<strong>${r.itemName}</strong>`,
      `<span style="color:#e11d48; font-weight:bold;">-${Number(r.quantityRemoved).toLocaleString()}</span>`,
  ...
  const totalItemsRemoved = rows.reduce((sum, r) => sum + (parseFloat(String(r.quantityRemoved)) || 0), 0);
...
<body>
  ...
  <div class="totals">Total Items Removed: <strong>${totalItemsRemoved.toLocaleString()}</strong></div>
  <script>window.onload = function() { window.print(); };</script>
</body>
</html>`;
}

export default function StockOutExporter({ schoolName, getExportRows }: Props) {
  ...
  const resolvedTitle = title.trim() || `Stock Out Ledger – ${new Date().toLocaleDateString('en-NG')}`;
  ...
  const handleExcel = async () => {
    ...
    try {
      ...
      const sheetRows = rows.map((r) => ({
        ...
        'Qty Removed': parseFloat(String(r.quantityRemoved)) || 0,
        'Reason': REASON_LABELS[r.reason] || r.reason,
        'Location': r.locationName,
        'Staff / Department': r.staffDept,
        'Date Removed': r.dateRemoved,
        'Remarks': r.notes,
      }));

      const ws = XLSX.utils.json_to_sheet(sheetRows);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Stock Out Ledger');
      XLSX.writeFile(wb, `stock_out_ledger_${new Date().toISOString().slice(0, 10)}.xlsx`);
  ...
  const handlePDF = async () => {
    ...
    try {
      const rows = await getExportRows();
      const html = buildPrintHTML(rows, resolvedTitle, schoolName);
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                <Download className="h-5 w-5 text-rose-600" /> Export Stock Removals
              </h3>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="flex items-center gap-3 justify-end pt-2 border-t border-slate-100">
                ...
                <button onClick={handleExcel} disabled={exporting} className="inline-flex items-center gap-2 px-5 py-2.5 bg-red-600 text-white text-sm font-semibold rounded-xl hover:bg-red-700 disabled:opacity-50">
