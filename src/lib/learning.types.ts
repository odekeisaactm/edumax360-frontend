...
export interface LearningResourcesSettings {
  ...
  // Null when no AI provider is attached. This is the common case for
  // schools without an AI subscription — the UI must guard against it.
...
export interface LessonMaterialList {
  ...
  created_at: string;
  // Requires 'is_active' added to LessonMaterialListSerializer.Meta.fields
  // on the backend. Until then, this arrives as undefined at runtime.
  is_active: boolean;
}
...
export type QuizExamScheduleStatus =
  | 'draft'
  | 'partial'
  | 'ready'
  | 'ongoing'
...
export interface QuizExamScheduleRef {
  id: number;
  class_configuration: number;
  class_configuration_name: string;
  setup_status: QuizExamScheduleStatus;
}
...
export interface AutoGeneratedQuizDetail {
  ...
  ai_generated: boolean;
  is_active: boolean;
  // One ExamScheduleModel per targeted class arm. Populated after
  // POST /quizzes/{id}/import-to-assessment/ runs. Empty before that.
  exam_schedules: QuizExamScheduleRef[];
...
export interface QuizImportToAssessmentResponse {
  imported_schedule_ids: number[];
  count: number;
}
