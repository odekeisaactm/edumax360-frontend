  ...
  return (
    <div className="relative" ref={containerRef}>
      <div className="relative">
        ...
        <input
          type="text"
          className={`${inputCls} pl-9 ${disabled ? 'bg-slate-50 cursor-not-allowed opacity-70' : ''}`}
          placeholder={loading ? 'Loading staff…' : selected ? staffLabel(selected) : 'Search staff by name or ID…'}
          value={open ? query : ''}
          onFocus={() => { if (!disabled) { setOpen(true); setHighlightIndex(0); } }}
...
function SalaryPreview({ setting, monthlySalary, additionalValues }: SalaryPreviewProps) {
  const calculation = useMemo(() => calculateSalary(monthlySalary, setting, additionalValues), [monthlySalary, setting, additionalValues]);
      ...
      <div className="p-4 space-y-4">
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-slate-50">
                ...
                <th className="text-right p-2 border border-slate-200 font-semibold text-slate-600">Monthly (₦)</th>
                <th className="text-right p-2 border border-slate-200 font-semibold text-slate-600">Annual (₦)</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td className="p-2 border border-slate-200 font-medium">Salary</td>
                <td className="p-2 border border-slate-200 text-right">{fmtMoney(monthlySalary)}</td>
                <td className="p-2 border border-slate-200 text-right">{fmtMoney(monthlySalary * 12)}</td>
              </tr>
              <tr>
                <td className="p-2 border border-slate-200 font-medium">Leave Allowance</td>
                <td className="p-2 border border-slate-200 text-right">{fmtMoney(calculation.leaveAllowanceMonthly)}</td>
                <td className="p-2 border border-slate-200 text-right">{fmtMoney(calculation.leaveAllowanceAnnual)}</td>
              </tr>
            </tbody>
          </table>
        </div>
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              ...
              <tr className="bg-slate-50">
                ...
                <th className="text-right p-2 border border-slate-200 font-semibold text-slate-600">Monthly (₦)</th>
                <th className="text-right p-2 border border-slate-200 font-semibold text-slate-600">Annual (₦)</th>
                <th className="text-right p-2 border border-slate-200 font-semibold text-slate-600">%</th>
              </tr>
            </thead>
            <tbody>
              {Object.values(calculation.basicComponents).map((comp: any) => (
                <tr key={comp.code}>
                  <td className="p-2 border border-slate-200">{comp.name}</td>
                  <td className="p-2 border border-slate-200 text-right">{fmtMoney(comp.monthly)}</td>
                  <td className="p-2 border border-slate-200 text-right">{fmtMoney(comp.annual)}</td>
                  <td className="p-2 border border-slate-200 text-right">{comp.percentage.toFixed(2)}%</td>
                </tr>
              ))}
              {calculation.allowances.map((a: any) => (
                <tr key={a.name}>
                  ...
                  <td className="p-2 border border-slate-200 text-right">
                    {monthlySalary > 0 ? ((a.monthly / monthlySalary) * 100).toFixed(2) : '0.00'}%
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-3">
          <div className="flex justify-between items-center">
            <span className="font-bold text-green-700">GROSS INCOME (ANNUAL)</span>
            <span className="font-bold text-green-700 text-lg">{fmtMoney(calculation.grossIncomeAnnual)}</span>
          </div>
        </div>
            ...
            <tbody>
              ...
              {calculation.reliefs.map((r: any) => (
                <tr key={r.name}>
                  ...
                  <td className="p-2 border border-slate-200 text-right">{fmtMoney(r.amount)}</td>
                </tr>
              ))}
              <tr className="bg-amber-100 font-bold">
                <td className="p-2 border border-slate-200">Tax Free Pay (Total Relief)</td>
                <td className="p-2 border border-slate-200 text-right">{fmtMoney(calculation.totalReliefs)}</td>
              </tr>
            </tbody>
          </table>
        </div>
          ...
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              ...
              {calculation.taxBreakdown.map((b: any, idx: number) => (
                <tr key={idx}>
                  <td className="p-2 border border-slate-200">{b.description}</td>
                  <td className="p-2 border border-slate-200 text-right">{fmtMoney(b.amount)}</td>
                  <td className="p-2 border border-slate-200 text-center text-xs text-slate-500">{b.rate}%</td>
                </tr>
              ))}
              <tr className="bg-red-100 font-bold">
                <td className="p-2 border border-slate-200">Total PAYE (Annual)</td>
                <td className="p-2 border border-slate-200 text-right">{fmtMoney(calculation.annualTax)}</td>
                <td className="p-2 border border-slate-200 text-right text-sm">{fmtMoney(calculation.monthlyTax)} / month</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export default function SalaryStructureEditPage() {
  ...
  // ── Load staff, settings, and existing structure ──
  useEffect(() => {
    if (!canEdit) return;
    (async () => {
      ...
      try {
        const [staffData, settingsData, structure] = await Promise.all([
  ...
  return (
    <div className="pb-28">
      ...
      <div className="flex items-center gap-3 mb-5">
        <button
          onClick={() => router.push(`/dashboard/staff/salary/structure/${structureId}`)}
        ...
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5 pl-12">
            {selectedStaff ? `Editing structure for ${staffLabel(selectedStaff)}` : `Structure #${structureId}`}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              {/* Staff is locked on edit — changing staff on an existing structure would be semantically wrong */}
            ...
            <div>
              ...
              >
                ...
                {salarySettings
                  .filter((s) => s.is_active || s.id === salarySettingId)
                  .map((s) => (
            ...
            <div>
              <label className={labelCls}>Effective From <span className="text-red-500 normal-case">*</span></label>
              <input
                type="date"
                className={inputCls}
                value={effectiveFrom}
        ...
        >
          {!staffId ? (
            <p className="text-sm text-slate-400">No staff selected.</p>
          ) : loadingBankDetail ? (
              ...
              {selectedSetting.additional_fields.map((field: any) => (
                <div key={field.code}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAdditionalFieldValues((prev) => ({ ...prev, [field.code]: parseFloat(e.target.value) || 0 }))
                    }
                  />
                </div>
              ))}
        ...
        <Section
          ...
          subtitle="Update monthly salary and see recalculated breakdown"
          required
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex items-center">
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 text-sm text-blue-700">
                ...
                All components recalculate automatically based on the selected salary setting.
              </div>
            </div>
          </div>
          ...
          <div className="px-5 py-3.5 flex items-center justify-between gap-4">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                <p className="text-xs font-bold text-slate-800 truncate">
                  {selectedStaff ? staffLabel(selectedStaff) : `Structure #${structureId}`}
                </p>
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-600 hover:to-orange-700 rounded-xl shadow-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {submitting ? (
                  <><Loader2 className="h-4 w-4 animate-spin" /> Saving…</>
                ) : (
...
// ─── Salary Calculation Engine (same as create page) ──────────────────────────
