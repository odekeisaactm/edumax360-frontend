...
import { staffAPI, academicCalendarAPI, staffLoansAPI } from '@/lib/api';
import {
  Landmark, Search, X, Check, AlertCircle, AlertTriangle, Loader2,
  RefreshCw, ChevronLeft, ChevronRight, UserCircle, CalendarDays,
  Wallet, XCircle, Plus, Eye, FileText, Printer, Building2, HandCoins, Users
} from 'lucide-react';
        ...
        {loading ? (
          ...
          <>
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 px-6 py-5 flex-shrink-0">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-slate-300">Loan Details</span>
                </div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {data.status === 'disbursed' && (
                  <span className="px-2.5 py-1 bg-rose-500/20 text-rose-300 rounded-full text-[11px] font-bold border border-rose-500/30">
                    Bal: {fmtMoney(data.balance)}
                  </span>
                )}
                {data.repaid_amount > 0 && (
                  <span className="px-2.5 py-1 bg-emerald-500/20 text-emerald-300 rounded-full text-[11px] font-bold border border-emerald-500/30">
                    Repaid: {fmtMoney(data.repaid_amount)}
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="p-6 space-y-5 flex-1 bg-slate-50/50">
              ...
              {/* Justification & Plan */}
              <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-1.5"><FileText className="h-3.5 w-3.5" /> Justification & Plan</p>
                <div className="space-y-4 mt-3">
                  <div>
                    <p className="text-xs font-semibold text-slate-700 mb-1">Reason:</p>
                    <p className="text-sm text-slate-600 whitespace-pre-wrap leading-relaxed">{data.reason}</p>
                  </div>
                  {data.repayment_plan && (
                    <div className="pt-3 border-t border-slate-100">
                      <p className="text-xs font-semibold text-slate-700 mb-1">Proposed Repayment Plan:</p>
                      <p className="text-sm text-slate-600 whitespace-pre-wrap leading-relaxed">{data.repayment_plan}</p>
                    </div>
                  )}
                </div>
              </div>
                ...
                <div className="space-y-3">
                  ...
                  {data.approved_date && (
                    <div className="flex justify-between items-center text-sm border-b border-slate-50 pb-2">
                      ...
                      <span className="font-semibold text-indigo-600">{new Date(data.approved_date).toLocaleDateString()}</span>
                    </div>
                  )}
...
// ─── Request Loan Modal ────────────────────────────────────────────────────────
function RequestLoanModal({ isSaving, currentPeriodId, onSave, onClose }: any) {
  const [form, setForm] = useState({
    staff: '', amount: '', reason: '', repayment_plan: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } as any); // Bypass strict interface
    } catch (err) { setFormError(extractError(err)); }
  };

  const inputCls = "w-full px-3.5 py-2.5 text-sm border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition bg-white";
    ...
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-gradient-to-br from-indigo-600 to-purple-600 rounded-lg flex items-center justify-center shadow-sm shadow-indigo-200"><Landmark className="h-4 w-4 text-white" /></div>
            <h3 className="text-base font-bold text-slate-900">Request Staff Loan</h3>
          </div>
        ...
        <form id="loan-form" onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          <div ref={searchRef}>
                ...
                <div className="absolute z-20 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-lg max-h-48 overflow-y-auto">
                  {staffResults.map(s => (
                    <button key={s.id} type="button"
                      ...
                      <div className="w-8 h-8 rounded-lg bg-indigo-50 flex items-center justify-center flex-shrink-0"><UserCircle className="h-4 w-4 text-indigo-400" /></div>
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Justification / Reason <span className="text-rose-400 normal-case">*</span></label>
            <textarea required value={form.reason} onChange={e => setForm(f => ({ ...f, reason: e.target.value }))} rows={2} placeholder="Why is this loan requested..." className={inputCls + ' resize-none'} />
          </div>

          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Proposed Repayment Plan</label>
            <textarea value={form.repayment_plan} onChange={e => setForm(f => ({ ...f, repayment_plan: e.target.value }))} rows={2} placeholder="e.g. ₦50,000 deducted across 10 months." className={inputCls + ' resize-none'} />
          </div>
        </form>
        ...
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 flex-shrink-0 bg-slate-50/50 rounded-b-2xl">
          ...
          <button type="submit" form="loan-form" disabled={isSaving} className="px-5 py-2 text-sm bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold rounded-xl hover:from-indigo-700 hover:to-purple-700 transition-all disabled:opacity-50 flex items-center gap-2 shadow-md shadow-indigo-200">
...
function LogRepaymentModal({ isSaving, onSave, onClose }: any) {
  const [form, setForm] = useState({ staff: '', amount_paid: '' });
  const [formError, setFormError] = useState<string | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.staff) return setFormError('Please select a staff member.');
    try {
      await onSave(Number(form.staff), form.amount_paid);
    } catch (err) { setFormError(extractError(err)); }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-lg flex items-center justify-center shadow-sm shadow-emerald-200"><HandCoins className="h-4 w-4 text-white" /></div>
            <h3 className="text-base font-bold text-slate-900">Log Cash Repayment</h3>
          </div>
          <button onClick={onClose} disabled={isSaving} className="text-slate-400 hover:text-slate-600 p-1.5 rounded-lg hover:bg-slate-100 transition-colors"><X className="h-5 w-5" /></button>
        </div>
        ...
        <form id="repay-form" onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="bg-emerald-50 border border-emerald-100 p-3 rounded-xl mb-2 text-xs text-emerald-700">
            This logs a physical cash or transfer payment. It will automatically sweep the amount against the staff's oldest active loan.
          </div>

          <div ref={searchRef}>
            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Staff Debtors <span className="text-rose-400 normal-case">*</span></label>
            <div className="relative">
              <input type="text" placeholder="Search debtor by name..." value={staffSearch}
                onChange={e => { setStaffSearch(e.target.value); setShowStaffDrop(true); setForm(f => ({ ...f, staff: '' })); }}
                onFocus={() => setShowStaffDrop(true)} className={inputCls + ' pr-10'} required={!form.staff} />
              <Search className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-300 pointer-events-none" />
              {showStaffDrop && staffResults.length > 0 && (
                <div className="absolute z-20 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-lg max-h-48 overflow-y-auto">
                  {staffResults.map(s => (
                    <button key={s.id} type="button"
                      onClick={() => { setForm(f => ({ ...f, staff: String(s.id) })); setStaffSearch(s.full_name || `${s.first_name} ${s.last_name}`); setShowStaffDrop(false); }}
                      className="w-full px-4 py-2.5 text-left hover:bg-slate-50 flex items-center gap-3 border-b border-slate-50 last:border-0">
                      <div className="w-8 h-8 rounded-lg bg-emerald-50 flex items-center justify-center flex-shrink-0"><UserCircle className="h-4 w-4 text-emerald-500" /></div>
                      <div><p className="text-sm font-bold text-slate-800">{s.full_name || `${s.first_name} ${s.last_name}`}</p><p className="text-xs text-slate-400 font-mono">{s.staff_id || `ID: ${s.id}`}</p></div>
                    </button>
                  ))}
                </div>
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Amount Paid (₦) <span className="text-rose-400 normal-case">*</span></label>
            <input type="number" step="0.01" min="0.01" required value={form.amount_paid} onChange={e => setForm(f => ({ ...f, amount_paid: e.target.value }))} placeholder="0.00" className={inputCls} />
          </div>
        </form>

        <div className="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 flex-shrink-0 bg-slate-50/50 rounded-b-2xl">
          <button type="button" onClick={onClose} disabled={isSaving} className="px-4 py-2 text-sm border border-slate-200 rounded-xl font-medium text-slate-600 hover:bg-slate-100 transition-colors">Cancel</button>
          <button type="submit" form="repay-form" disabled={isSaving} className="px-5 py-2 text-sm bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-semibold rounded-xl hover:from-emerald-600 hover:to-teal-700 transition-all disabled:opacity-50 flex items-center gap-2 shadow-md shadow-emerald-200">
            {isSaving ? <><Loader2 className="h-4 w-4 animate-spin" /> Logging...</> : <><HandCoins className="h-4 w-4" /> Log Payment</>}
          </button>
        </div>
      </div>
    </div>
...
export default function StaffLoansPage() {
  ...
  const canCreate = user?.is_superuser || hasPermission('salary_management.add_staffloanmodel');
  ...
  const fetchData = useCallback(async (pg = 1, fetchAllForPrint = false) => {
    ...
    try {
      ...
      const res = await staffLoansAPI.list(params) as any;

      if (fetchAllForPrint) return unwrapList(res);

      setLoans(unwrapList(res));
  ...
  // ── Actions ──
  const handleSaveLoan = async (payload: any) => {
    setIsSaving(true);
    try {
      await staffLoansAPI.create(payload);
      showToast('success', 'Staff loan requested successfully');
      setShowCreate(false);
      fetchData(1);
    } catch (err) { throw err; } finally { setIsSaving(false); }
  };

  const handleRepay = async (staffId: number, amount: string) => {
    setIsSaving(true);
    try {
      await staffLoansAPI.recordRepayment(staffId, { amount_paid: parseFloat(amount) });
      showToast('success', 'Manual repayment logged successfully');
      setShowRepay(false);
      fetchData(page);
    } catch (err) { throw err; } finally { setIsSaving(false); }
  };
  ...
  const processAction = async () => {
    ...
    try {
      await staffLoansAPI.action(actionModal.id, { action: actionModal.action as any });
      showToast('success', `Loan ${actionModal.action}d successfully`);
  ...
  const totalActiveDebt = loans.filter(l => l.status === 'disbursed').reduce((s, l) => s + parseFloat(l.balance || l.amount), 0);
  const totalRecovered = loans.reduce((s, l) => s + parseFloat(l.repaid_amount || '0'), 0);

  const selectCls = "px-3 py-2 text-sm border border-slate-200 rounded-xl outline-none bg-white text-slate-600 focus:ring-2 focus:ring-indigo-500 transition-all";
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <LoanDetailDrawer loanId={detailId} onClose={() => setDetailId(null)} />
      ...
      <ConfirmActionModal
        ...
        colorCls={actionModal.action === 'reject' ? 'bg-rose-600 hover:bg-rose-700' : actionModal.action === 'approve' ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-blue-600 hover:bg-blue-700'}
        isProcessing={isProcessingAction} onConfirm={processAction} onCancel={() => setActionModal({ ...actionModal, open: false })}
      />

      {showCreate && <RequestLoanModal isSaving={isSaving} currentPeriodId={currentPeriodId} onSave={handleSaveLoan} onClose={() => setShowCreate(false)} />}
      {showRepay && <LogRepaymentModal isSaving={isSaving} onSave={handleRepay} onClose={() => setShowRepay(false)} />}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Staff Loans</h1>
            <p className="text-sm text-slate-400 mt-0.5">Manage long-term structured deductions and manual payments</p>
          </div>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {canApprove && (
            <button onClick={() => setShowRepay(true)} className="inline-flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-600 text-white text-sm font-bold rounded-xl hover:from-emerald-600 hover:to-teal-700 transition-all shadow-md shadow-emerald-200">
              <HandCoins className="h-4 w-4" /> Log Payment
            </button>
          )}
          {canCreate && (
            <button onClick={() => setShowCreate(true)} className="inline-flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white text-sm font-bold rounded-xl hover:from-indigo-700 hover:to-purple-700 transition-all shadow-md shadow-indigo-200">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Total Active Debt', value: fmtMoney(totalActiveDebt), iconBg: 'bg-rose-50 border-rose-100', iconColor: 'text-rose-500', icon: Users },
          ...
          { label: 'Total Loans', value: total, iconBg: 'bg-indigo-50 border-indigo-100', iconColor: 'text-indigo-500', icon: Landmark },
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div className="px-5 py-4 border-b border-slate-50 bg-slate-50/50 space-y-4">
          <div className="flex flex-col lg:flex-row lg:items-center gap-4">
            ...
            <div className="relative flex-1 w-full">
              ...
              <input type="text" placeholder="Search by staff name or ID..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-9 pr-8 py-2.5 text-sm border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition-all bg-white shadow-sm" />
            ...
            {/* Mode & Dropdowns */}
            <div className="flex items-center gap-3 flex-wrap">
              {/* Debtors Quick Filter */}
              <button onClick={() => setStatusFilter(statusFilter === 'disbursed' ? '' : 'disbursed')} className={`px-4 py-2.5 text-sm font-bold rounded-xl border transition-all ${statusFilter === 'disbursed' ? 'bg-rose-50 text-rose-600 border-rose-200 shadow-sm' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}>
                Active Debtors
              </button>

              {/* Dual Filter Toggle */}
              <div className="flex bg-slate-200/60 p-1 rounded-xl border border-slate-200">
                <button onClick={() => setFilterMode('month')} className={`px-4 py-1.5 text-xs font-bold rounded-lg transition-all ${filterMode === 'month' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>By Month</button>
                <button onClick={() => setFilterMode('period')} className={`px-4 py-1.5 text-xs font-bold rounded-lg transition-all ${filterMode === 'period' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>By Term</button>
              </div>
              ...
              <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className={selectCls}>
                ...
                <option value="disbursed">Disbursed (Debts)</option>
        ...
        {/* List */}
        {loading ? (
          <div className="p-20 text-center"><Loader2 className="h-8 w-8 animate-spin text-indigo-500 mx-auto" /><p className="mt-3 text-sm font-medium text-slate-400">Loading loans...</p></div>
        ) : pageError ? (
          <div className="p-16 text-center"><AlertCircle className="h-8 w-8 text-rose-400 mx-auto mb-3" /><p className="text-sm font-medium text-rose-600 mb-4">{pageError}</p><button onClick={() => fetchData(1)} className="text-sm text-indigo-600 font-bold hover:underline inline-flex items-center gap-1.5"><RefreshCw className="h-4 w-4" /> Retry</button></div>
        ) : loans.length === 0 ? (
          <div className="p-20 text-center">
            <div className="w-16 h-16 bg-indigo-50 border border-indigo-100 rounded-2xl flex items-center justify-center mx-auto mb-5"><Landmark className="h-8 w-8 text-indigo-400" /></div>
            <h3 className="text-lg font-bold text-slate-800 mb-1">No staff loans found</h3>
            <p className="text-sm text-slate-500">There are no staff loan records matching your current filters.</p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full border-collapse" style={{ tableLayout: 'fixed', minWidth: '950px' }}>
                ...
                <thead>
                  <tr className="bg-slate-50/80 border-b border-slate-100">
                    ...
                    <th className="px-5 py-3 text-right text-[11px] font-bold text-slate-400 uppercase tracking-widest">Loan Amount</th>
                    <th className="px-5 py-3 text-right text-[11px] font-bold text-slate-400 uppercase tracking-widest">Balance</th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {loans.map(loan => (
                    <tr key={loan.id} className="hover:bg-slate-50/60 transition-colors group">
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="w-9 h-9 rounded-lg bg-indigo-50 border border-indigo-100 flex items-center justify-center flex-shrink-0 text-indigo-500"><UserCircle className="h-5 w-5" /></div>
                          <div className="min-w-0">
                            <p className="text-sm font-bold text-slate-900 truncate group-hover:text-indigo-600 transition-colors">{loan.staff_name || 'Unknown'}</p>
                            <p className="text-[11px] text-slate-400 font-mono truncate">{loan.staff_detail?.staff_id || ''}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-3.5 text-right">
                        <p className="text-[15px] font-black text-slate-800 tabular-nums">{fmtMoney(loan.amount)}</p>
                      </td>
                      <td className="px-5 py-3.5 text-right">
                        {loan.status === 'disbursed' ? (
                          <p className="text-[14px] font-black text-rose-500 tabular-nums">{fmtMoney(loan.balance)}</p>
                        ) : loan.status === 'completed' ? (
                          <p className="text-[14px] font-black text-emerald-500 tabular-nums">₦0.00</p>
                        ) : (
                          <p className="text-xs font-semibold text-slate-400">—</p>
                        )}
                      </td>
                      <td className="px-5 py-3.5">
                        <p className="text-xs font-medium text-slate-500 truncate" title={loan.reason}>{loan.reason}</p>
                      </td>
                      <td className="px-5 py-3.5">
                        <span className="text-xs font-semibold text-slate-600">{new Date(loan.request_date).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric'})}</span>
                      </td>
                      <td className="px-5 py-3.5 text-center">
                        <span className={`inline-flex px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-md border ${STATUS_CONFIG[loan.status]?.pill || 'bg-slate-100 text-slate-500'}`}>
                          {STATUS_CONFIG[loan.status]?.label || loan.status}
                        </span>
                      </td>
                      <td className="px-5 py-3.5">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => setDetailId(loan.id)} title="View Details" className="w-8 h-8 rounded-lg bg-slate-50 border border-slate-200 text-slate-600 flex items-center justify-center hover:bg-slate-100 transition-colors"><Eye className="h-4 w-4"/></button>

                          {canApprove && loan.status === 'pending' && (
                            <>
                              <button onClick={() => setActionModal({ open: true, title: 'Approve Loan', message: `Approve ${fmtMoney(loan.amount)} for ${loan.staff_name}?`, action: 'approve', id: loan.id })} className="w-8 h-8 rounded-lg bg-indigo-50 border border-indigo-200 text-indigo-600 flex items-center justify-center hover:bg-indigo-100 transition-colors" title="Approve"><Check className="w-4 h-4"/></button>
                              <button onClick={() => setActionModal({ open: true, title: 'Reject Loan', message: `Are you sure you want to reject this loan?`, action: 'reject', id: loan.id })} className="w-8 h-8 rounded-lg bg-rose-50 border border-rose-200 text-rose-600 flex items-center justify-center hover:bg-rose-100 transition-colors" title="Reject"><XCircle className="w-4 h-4"/></button>
                            </>
                          )}

                          {canApprove && loan.status === 'approved' && (
                            <button onClick={() => setActionModal({ open: true, title: 'Disburse Funds', message: `Confirm ${fmtMoney(loan.amount)} has been disbursed to ${loan.staff_name}? It will now appear on their payroll as an active debt.`, action: 'disburse', id: loan.id })} className="px-3 py-1.5 rounded-lg bg-blue-50 border border-blue-200 text-blue-600 flex items-center justify-center gap-1.5 text-xs font-bold hover:bg-blue-100 transition-colors"><Wallet className="w-3.5 h-3.5"/> Disburse</button>
                          )}
                        </div>
                      </td>
                    </tr>
            ...
            <div className="px-5 py-3 border-t border-slate-100 bg-slate-50/40 flex items-center justify-between">
              <p className="text-xs text-slate-400">Showing <span className="font-semibold text-slate-600">{loans.length > 0 ? (page - 1) * PAGE_SIZE + 1 : 0}–{Math.min(page * PAGE_SIZE, total)}</span> of <span className="font-bold text-slate-700">{total}</span> records</p>
      ...
      {isPrinting && (
        <div className="fixed inset-0 z-[100] bg-slate-900/50 backdrop-blur-sm flex items-start justify-center overflow-y-auto py-8 px-4 print:p-0 print:bg-white animate-in fade-in">
          <div id="receipt-print-area" className="bg-white w-full max-w-5xl rounded-2xl shadow-2xl overflow-hidden print:shadow-none print:rounded-none print:max-w-none print:w-full">
              ...
              <div className="flex items-center gap-4 pb-4 border-b-2 border-slate-900 mb-6">
                ...
                <div className="shrink-0 text-right">
                  ...
                  <span className="text-[12px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full bg-slate-100 text-slate-800 whitespace-nowrap">
                    Staff Loans {statusFilter === 'disbursed' ? '(Debtors)' : ''}
                  </span>
                </div>
              </div>
                ...
                <table className="w-full text-sm border-collapse border border-slate-300">
                  <thead className="bg-slate-100">
                    <tr>
                      ...
                      <th className="border border-slate-300 px-4 py-2 text-right font-bold text-slate-800 uppercase text-[11px] tracking-wider w-32">Loan Amount</th>
                      <th className="border border-slate-300 px-4 py-2 text-right font-bold text-slate-800 uppercase text-[11px] tracking-wider w-32">Balance Due</th>
                    </tr>
                  </thead>
                  <tbody>
                    {printData.map((loan, i) => (
                      <tr key={loan.id}>
                        <td className="border border-slate-300 px-4 py-2 text-center text-slate-600">{i + 1}</td>
                        <td className="border border-slate-300 px-4 py-2 font-medium text-slate-800">{loan.staff_name}</td>
                        <td className="border border-slate-300 px-4 py-2 font-medium text-slate-800 uppercase text-[10px]">{loan.status}</td>
                        <td className="border border-slate-300 px-4 py-2 text-right font-bold text-slate-900 tabular-nums">{fmtMoney(loan.amount)}</td>
                        <td className="border border-slate-300 px-4 py-2 text-right font-bold text-rose-600 tabular-nums">{loan.status === 'disbursed' ? fmtMoney(loan.balance) : '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                  {printData.length === 0 && (
                  ...
                  {printData.length > 0 && statusFilter === 'disbursed' && (
                     <tfoot>
                       <tr className="bg-slate-50 font-black">
                         <td colSpan={4} className="border border-slate-300 px-4 py-3 text-right text-slate-800 uppercase text-[11px] tracking-wider">Total Active Debt</td>
                         <td className="border border-slate-300 px-4 py-3 text-right text-rose-600 tabular-nums text-base">{fmtMoney(totalActiveDebt)}</td>
                       </tr>
                     </tfoot>
                  )}
                </table>
