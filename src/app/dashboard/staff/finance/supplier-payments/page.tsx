...
import { supplierPaymentsAPI } from '@/lib/finance.service';
import type { SupplierPayment } from '@/lib/finance.types';
import {
  CreditCard, Plus, Search, X, AlertCircle, Loader2, RefreshCw,
  ChevronLeft, ChevronRight, Filter, Eye, Printer
} from 'lucide-react';
...
export default function SupplierPaymentsPage() {
  ...
  const canCreate = user?.is_superuser || hasPermission('finance.add_supplierpaymentmodel');
  ...
  const fetchPayments = useCallback(async (pg = 1) => {
    ...
    try {
      ...
      const data = await supplierPaymentsAPI.list(params);
      let results: SupplierPayment[] = [];
      let totalCount = 0;
  ...
  // Fallback for cases where `count` isn't reliably returned by the API.
    ...
    <div className="max-w-7xl mx-auto space-y-5 pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1 pl-12">Payments made to suppliers for purchase orders</p>
        </div>
        {canCreate && (
          <button
            onClick={() => router.push('/dashboard/staff/finance/supplier-payments/new')}
      ...
      {loading ? (
        <div className="p-16 text-center">
          ...
          <p className="mt-2 text-sm text-slate-400">Loading supplier payments...</p>
        </div>
      ) : pageError ? (
        ...
        <div className="p-16 text-center">
          ...
          <h3 className="font-semibold text-slate-700 mb-1">No supplier payments found</h3>
              ...
              <tbody className="divide-y divide-slate-100">
                {payments.map(p => (
                  <tr key={p.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3 font-mono text-indigo-600">{p.receipt_number}</td>
                    <td className="px-4 py-3 font-medium text-slate-800">{p.supplier_name}</td>
                    <td className="px-4 py-3 font-mono text-xs text-slate-500">{(p as any).purchase_order_number || '—'}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          onClick={() => router.push(`/dashboard/staff/finance/supplier-payments/${p.id}`)}
                          title="View Details"
                        ...
                        <button
                          onClick={() => router.push(`/dashboard/staff/finance/supplier-payments/${p.id}?print=receipt`)}
                          title="Print Receipt"
