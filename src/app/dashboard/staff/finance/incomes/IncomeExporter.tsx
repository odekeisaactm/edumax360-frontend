...
function buildPrintHTML(rows: ExportRow[], title: string, schoolName?: string, filterSummary?: string, baseCurrency = '₦'): string {
  const now = new Date().toLocaleString('en-NG', { dateStyle: 'long', timeStyle: 'short' });
  const headers = ['#', 'Category', 'Source', 'Destination Account', 'Method', 'Amount', 'Date', 'Ref'];
  ...
  const bodyRows = rows.map((r, i) => {
    const cells = [
      ...
      r.source,
      r.bankAccountName,
      `<span style="text-transform:capitalize">${r.paymentMethod}</span>`,
      `<strong>${fmtMoney(r.amount, baseCurrency)}</strong>`,
      r.incomeDate,
      `<span style="font-family:monospace">${r.reference}</span>`,
    ];
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .header .school { font-size: 13px; font-weight: 700; color: #2563eb; margin-bottom: 4px; }
    ...
    th { background: #2563eb; color: #fff; padding: 7px 8px; text-align: left; font-size: 9.5px; font-weight: 700; text-transform: uppercase; }
    td { padding: 6px 8px; border-bottom: 1px solid #f1f5f9; vertical-align: middle; }
    tr.even td { background: #f8fafc; }
    .totals { margin-top: 14px; text-align: right; font-size: 11px; }
    .totals strong { color: #2563eb; font-size: 13px; }
...
<body>
  <div class="header">
    ...
    <h1>${title}</h1>
    <div class="meta">Generated: ${now} &nbsp;|&nbsp; Total inflows: ${rows.length}</div>
  </div>
  <table>
    <thead><tr>${headers.map(h => `<th>${h}</th>`).join('')}</tr></thead>
    <tbody>${bodyRows}</tbody>
  </table>
  <div class="totals">Total Revenue Recorded: <strong>${fmtMoney(totalAmount, baseCurrency)}</strong></div>
  <script>window.onload = function() { window.print(); };</script>
</body>
</html>`;
}

export default function IncomeExporter({ schoolName, filterSummary, getExportRows, baseCurrency = '₦' }: Props) {
  ...
  const resolvedTitle = title.trim() || `Master Revenue Ledger – ${new Date().toLocaleDateString('en-NG')}`;
  ...
  const handleExcel = async () => {
    ...
    try {
      ...
      const sheetRows = rows.map((r) => ({
        ...
        'Received From':    r.source,
        'Destination':      r.bankAccountName,
        'Payment Method':   r.paymentMethod,
        'Amount Recorded':  parseFloat(String(r.amount)) || 0,
        'Date Received':    r.incomeDate,
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Inflow Ledger');
      XLSX.writeFile(wb, `revenue_inflows_${new Date().toISOString().slice(0, 10)}.xlsx`);
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                <Download className="h-5 w-5 text-blue-600" /> Export Revenue Inflows
              </h3>
