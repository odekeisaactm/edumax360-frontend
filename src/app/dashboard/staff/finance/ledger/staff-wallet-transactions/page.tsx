...
import type { StaffWalletTransaction } from '@/lib/finance.types';
import {
...
// Types whose amount represents money leaving the wallet — shown negative/red.
const OUTFLOW_TYPES = new Set(['deduction']);

// Staff filter component (similar to PersonFilterBox but only staff)
function StaffFilterBox({
  value,
  onSelect,
  onClear,
}: {
  value: { id: number; name: string } | null;
  onSelect: (s: { id: number; name: string }) => void;
  onClear: () => void;
}) {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      setLoading(true);
      staffAPI.list({ search: query.trim(), page_size: 8 })
        .then((data: any) => {
              ...
              results.map(p => (
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-slate-400 font-mono truncate">{p.staff_id || ''}</p>
                  </div>
                </button>
              ))
            )}
...
function TransactionDetailDrawer({ transactionId, onClose }: { transactionId: number | null; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    setError(null);
    setTx(null);
    // Real detail/retrieve call — was previously calling the LIST endpoint with
    // an `id` query param the backend never read, so `tx` ended up being the
    // paginated {count, results:[...]} envelope instead of a single transaction.
    auditLedgersAPI.getStaffWalletTransaction(transactionId)
      .then(setTx)
    ...
    <div className="fixed inset-0 z-[90] flex justify-end">
      ...
      <div className="relative bg-white w-full max-w-xl h-full shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col">
        <div className="px-6 py-4 bg-gradient-to-r from-indigo-600 to-purple-700 text-white flex items-center justify-between">
          <h3 className="font-bold">Staff Wallet Transaction</h3>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><span className="text-slate-500">Staff:</span><span className="font-medium ml-1">{tx.staff_name}</span></div>
...
export default function StaffWalletTransactionsPage() {
  const router = useRouter();
  const { hasPermission } = useAuth();

  const [transactions, setTransactions] = useState<StaffWalletTransaction[]>([]);
  ...
  const [staffFilter, setStaffFilter] = useState<{ id: number; name: string } | null>(null);
  ...
  const fetchTransactions = useCallback(async (pg = 1) => {
    ...
    try {
      ...
      if (staffFilter) params.staff_id = staffFilter.id;
      if (typeFilter) params.transaction_type = typeFilter;
      if (dateFrom) params.date_from = dateFrom;
      if (dateTo) params.date_to = dateTo;

      const data = await auditLedgersAPI.getStaffWalletLedger(params);
      let results: StaffWalletTransaction[] = [];
      let totalCount = 0;
  ...
  }, [search, staffFilter, typeFilter, dateFrom, dateTo]);
  ...
  // Re-fetch when staff filter changes immediately
  useEffect(() => {
    fetchTransactions(1);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [staffFilter]);
    ...
    <div className="max-w-7xl mx-auto space-y-5 pb-10">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1 pl-12">Staff wallet transactions</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[220px]">
          ...
          <input
            type="text"
            placeholder="Search staff, reason, or reference..."
            value={search}
        ...
        <StaffFilterBox value={staffFilter} onSelect={setStaffFilter} onClear={() => setStaffFilter(null)} />
      ...
      {/* List */}
      {loading ? (
        <div className="p-16 text-center"><Loader2 className="h-8 w-8 animate-spin text-indigo-600 mx-auto" /><p className="mt-2 text-sm text-slate-400">Loading staff wallet transactions...</p></div>
      ) : pageError ? (
                ...
                {transactions.map(t => {
                  ...
                  return (
                    <tr key={t.id} className="hover:bg-slate-50 cursor-pointer" onClick={() => setSelectedTxId(t.id)}>
                      ...
                      <td className="px-4 py-3 font-medium text-slate-800">{t.staff_name}</td>
                      <td className="px-4 py-3">
