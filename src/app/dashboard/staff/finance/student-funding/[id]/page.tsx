...
import { StudentFunding } from '@/lib/types';
import {
  ArrowLeft,
  Loader2,
  AlertCircle,
...
function buildReceiptHTML(
  ...
  const studentName = student
    ? toTitleCase(student.full_name || `${student.first_name || ''} ${student.last_name || ''}`.trim())
    : '—';
  const studentClass = student
    ? [student.current_class_name, student.current_class_section_name].filter(Boolean).join(' · ')
    : '—';
  ...
  const rows = [
    ...
    ['Class',             studentClass],
    ['Wallet Type',       (funding.wallet_type || '—').charAt(0).toUpperCase() + (funding.wallet_type || '').slice(1)],
...
<head>
  <meta charset="UTF-8"/>
  <title>Funding Receipt #${funding.id}</title>
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .school-name {
      font-size: 20px; font-weight: 800; color: #059669; letter-spacing: -0.3px; margin-bottom: 4px;
    }
    .receipt-title {
...
// ─── Main Page ─────────────────────────────────────────────────────────────────
export default function StudentFundingDetailPage() {
  const router = useRouter();
  const params = useParams();
  const id = Number(params.id);
  const { hasPermission, user, schoolInfo } = useAuth();

  const [funding, setFunding]   = useState<StudentFunding | null>(null);
  ...
  const canConfirm = user?.is_superuser || hasPermission('finance.change_studentfundingmodel');
  const canView    = user?.is_superuser || hasPermission('finance.view_studentfundingmodel');
  ...
  const fetchFunding = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await studentFundingAPI.get(id);
      setFunding(data);
      if (data.student) {
        try {
          const s = await studentsAPI.get(data.student);
          setStudent(s);
        } catch { /* silent */ }
      }
    } catch (err) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      await studentFundingAPI.action(id, { action: 'confirm' });
  ...
  const handleDecline = async () => {
    ...
    try {
      await studentFundingAPI.action(id, { action: 'decline', reason: declineReason });
  ...
  const handleRevert = async () => {
    ...
    try {
      await studentFundingAPI.action(id, { action: 'revert', reason: revertReason });
  ...
  const handlePrintReceipt = () => {
    if (!funding) return;
    const html = buildReceiptHTML(funding, student, schoolInfo?.name || 'School');
    ...
    <div className="pb-10 max-w-4xl mx-auto">
      ...
      {showRevertModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="p-6 space-y-4">
              <p className="text-sm text-slate-600">This will deduct the amount from the student's wallet. Please provide a reason.</p>
              <textarea
                value={revertReason}
      ...
      <div className="flex items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          <button
            onClick={() => router.push('/dashboard/staff/finance/deposits?filter=student')}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div className="p-6 divide-y divide-slate-50">
              <InfoRow label="Student"             value={studentName}                                                         icon={User} />
              <InfoRow label="Registration Number" value={student?.registration_number || '—'}                                  icon={User} />
              <InfoRow label="Class"               value={student ? [student.current_class_name, student.current_class_section_name].filter(Boolean).join(' · ') : '—'} icon={Calendar} />
              <InfoRow label="Wallet Type"         value={funding.wallet_type ? <span className="capitalize">{funding.wallet_type}</span> : '—'} icon={funding.wallet_type === 'canteen' ? Wallet : CreditCard} />
        ...
        <div className="space-y-5">
          ...
          {student && (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-50">
                <h3 className="text-sm font-bold text-slate-800">Student Info</h3>
              </div>
