...
import {
  ShoppingCart, Plus, Search, X, AlertCircle, Loader2,
  RefreshCw, ChevronLeft, ChevronRight, Eye, Building2,
  CalendarDays, Check, Filter
} from 'lucide-react';
...
// Assuming this interface based on your models & serializers
...
export default function PurchaseOrderListPage() {
  ...
  const fetchOrders = useCallback(async (search: string, statusFilter: string, pg = 1) => {
    setLoading(true);
    setPageError(null);
    try {
  ...
  useEffect(() => {
    ...
    searchDebounce.current = setTimeout(() => {
      fetchOrders(pendingSearch, selectedStatus, 1);
    }, 400);
    ...
    <div className="space-y-5 pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1 pl-12">Manage supplier orders and procurement</p>
        </div>

        <div className="flex items-center gap-2 w-full sm:w-auto">
          {canManage && (
            <button
              onClick={() => router.push('/dashboard/staff/inventory/purchase-orders/new')}
              className="inline-flex flex-1 sm:flex-none justify-center items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-700 text-white text-sm font-semibold rounded-xl hover:from-blue-700 hover:to-indigo-800 transition-all shadow-md shadow-blue-200"
            >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Total Orders', value: total, color: 'from-blue-600 to-indigo-700', shadow: 'shadow-blue-100' },
          { label: 'This Page', value: orders.length, color: 'from-slate-500 to-slate-600', shadow: 'shadow-slate-100' },
          { label: 'Pending Drafts', value: orders.filter(o => o.status === 'draft').length, color: 'from-gray-400 to-slate-500', shadow: 'shadow-gray-100' },
          { label: 'Page Total Value', value: `₦${orders.reduce((sum, o) => sum + Number(o.total_amount || 0), 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, color: 'from-emerald-500 to-teal-600', shadow: 'shadow-emerald-100' },
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            ...
            <div className="relative w-full sm:w-auto">
              <select
                ...
                className="w-full sm:w-auto pl-9 pr-9 py-2 text-sm border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none appearance-none bg-white"
              >
            ...
            <button
              onClick={() => fetchOrders(pendingSearch, selectedStatus, page)}
              title="Refresh"
        ...
        {loading ? (
          <div className="p-16 text-center">
            ...
            <p className="mt-2 text-sm text-slate-400">Loading purchase orders...</p>
          </div>
        ) : pageError ? (
          <div className="p-10 text-center">
            <AlertCircle className="h-8 w-8 text-red-400 mx-auto mb-2" />
            <p className="text-sm text-red-600 mb-3">{pageError}</p>
            <button onClick={() => fetchOrders(pendingSearch, selectedStatus, 1)}
          ...
          <div className="p-16 text-center">
            ...
            <h3 className="font-semibold text-slate-700 mb-1">No purchase orders found</h3>
            <p className="text-sm text-slate-400 mb-5">
              {pendingSearch || selectedStatus ? 'Try adjusting your filters.' : 'Create your first purchase order to start procuring stock.'}
            </p>
            {!pendingSearch && !selectedStatus && canManage && (
              <button
                onClick={() => router.push('/dashboard/staff/inventory/purchase-orders/new')}
                className="inline-flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-700 text-white text-sm font-semibold rounded-xl shadow-md shadow-blue-200"
              >
                <Plus className="h-4 w-4" /> New Order
              </button>
            )}
          ...
          <>
            ...
            <div
              ...
              style={{ gridTemplateColumns: '2.5rem 1.5fr 1.5fr 1fr 120px 100px 56px' }}
            >
              <span />
            ...
            <div className="divide-y divide-slate-50">
              {orders.map(order => (
                ...
                >
                  ...
                  {/* Order Number */}
                  <div className="min-w-0">
                    <p className="font-semibold text-slate-900 text-sm font-mono truncate">{order.order_number}</p>
                  ...
                  <div className="hidden sm:flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-xs font-medium text-slate-700 truncate">{order.supplier_name || '—'}</span>
                  </div>
                  ...
                  <div className="hidden sm:flex justify-center">
                    <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-bold border uppercase tracking-wider ${STATUS_STYLES[order.status] || STATUS_STYLES.draft}`}>
                      {STATUS_LABELS[order.status] || order.status}
                    </span>
                  </div>

                  {/* Mobile Only Meta View */}
                  <div className="sm:hidden flex-1 flex flex-col items-end gap-1 min-w-0">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-bold border uppercase tracking-wider ${STATUS_STYLES[order.status] || STATUS_STYLES.draft}`}>
                      {STATUS_LABELS[order.status] || order.status}
                    </span>
                  ...
                  <div className="flex items-center justify-end">
                    <button
                      onClick={() => router.push(`/dashboard/staff/inventory/purchase-orders/${order.id}`)}
                      title="View"
            ...
            <div className="px-5 py-3 border-t border-slate-50 bg-slate-50/40 flex items-center justify-between gap-4 flex-wrap">
              ...
              {totalPages > 1 && (
                <div className="flex items-center gap-1">
                  <button
                    onClick={() => fetchOrders(pendingSearch, selectedStatus, page - 1)}
                    disabled={page === 1}
                  ...
                  {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                    ...
                    return (
                      <button
                        key={pg}
                        onClick={() => fetchOrders(pendingSearch, selectedStatus, pg)}
