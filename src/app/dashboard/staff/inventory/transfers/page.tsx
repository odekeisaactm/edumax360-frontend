...
import { stockTransferAPI, inventoryItemAPI, schoolInfoAPI } from '@/lib/api';
import { StockTransfer, InventoryItem } from '@/lib/types';
import {
  ArrowLeftRight, Plus, Search, X, AlertCircle, Loader2,
  RefreshCw, ChevronLeft, ChevronRight, Eye, MapPin,
  CalendarDays, ReceiptText, Check, ArrowRight, Tag,
} from 'lucide-react';
...
export default function StockTransferListPage() {
  ...
  const fetchTransfers = useCallback(async (search: string, itemId: number | undefined, pg = 1) => {
    setLoading(true);
    setPageError(null);
    try {
    ...
    if (urlItemId) {
      inventoryItemAPI.get(Number(urlItemId))
        .then((itemData: InventoryItem) => {
          ...
          fetchTransfers('', itemData.id, 1);
        })
        .catch(() => {
          showToast('error', 'Could not load filtered item details.');
          fetchTransfers('', undefined, 1);
        });
    } else {
      fetchTransfers('', undefined, 1);
    }
  ...
  useEffect(() => {
    ...
    searchDebounce.current = setTimeout(() => {
      fetchTransfers(pendingSearch, selectedItem?.id, 1);
    }, 400);
    return () => { if (searchDebounce.current) clearTimeout(searchDebounce.current); };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pendingSearch, selectedItem]);
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          {canManage && (
            <button
              ...
              className="inline-flex flex-1 sm:flex-none justify-center items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-violet-500 to-purple-600 text-white text-sm font-semibold rounded-xl hover:from-violet-600 hover:to-purple-700 transition-all shadow-md shadow-violet-200"
            >
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div className="px-5 py-4 border-b border-slate-50">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            ...
            <div className="relative flex-1 w-full z-10">
              {selectedItem ? (
                <div className="flex items-center justify-between w-full px-3 py-2 text-sm border border-violet-200 bg-violet-50 text-violet-800 rounded-xl">
                  ...
                  <button onClick={() => { setSelectedItem(null); setItemSearchQuery(''); }}
                    className="p-0.5 hover:bg-violet-200 rounded-md transition-colors flex-shrink-0 ml-2">
            ...
            <button
              onClick={() => fetchTransfers(pendingSearch, selectedItem?.id, page)}
              title="Refresh"
        ...
        {loading ? (
          ...
          <div className="p-10 text-center">
            ...
            <button onClick={() => fetchTransfers(pendingSearch, selectedItem?.id, 1)}
          ...
          <div className="p-16 text-center">
            ...
            <p className="text-sm text-slate-400 mb-5">
              {pendingSearch || selectedItem ? 'Try adjusting your filters.' : 'Record your first stock transfer to get started.'}
            </p>
            ...
            <div className="px-5 py-3 border-t border-slate-50 bg-slate-50/40 flex items-center justify-between gap-4 flex-wrap">
              ...
              {totalPages > 1 && (
                <div className="flex items-center gap-1">
                  <button
                    onClick={() => fetchTransfers(pendingSearch, selectedItem?.id, page - 1)}
                    disabled={page === 1}
                  ...
                  {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                    ...
                    return (
                      <button
                        key={pg}
                        onClick={() => fetchTransfers(pendingSearch, selectedItem?.id, pg)}
