...
export interface DebtorExportRow {
  id: number;
  name: string;
  type: 'Parent' | 'Student';
  contactOrClass: string;
...
interface Props {
  ...
  getExportRows: () => Promise<DebtorExportRow[]>;
}
...
function buildPrintHTML(rows: DebtorExportRow[], title: string, schoolName?: string, filterSummary?: string): string {
  const now = new Date().toLocaleString('en-NG', { dateStyle: 'long', timeStyle: 'short' });
  const headers = ['#', 'Debtor Name', 'Type', 'Contact / Class', 'Total Outstanding'];
  ...
  const bodyRows = rows.map((r, i) => {
    const cells = [
      ...
      r.type,
      r.contactOrClass,
      `<strong style="color: #e11d48;">${fmtMoney(r.totalOwed)}</strong>`,
    ];
    return `<tr class="${i % 2 === 0 ? 'even' : ''}">${cells.map(c => `<td>${c}</td>`).join('')}</tr>`;
  }).join('');

  const totalDebt = rows.reduce((sum, r) => sum + (parseFloat(String(r.totalOwed)) || 0), 0);
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .header { text-align: center; margin-bottom: 18px; border-bottom: 2px solid #e11d48; padding-bottom: 14px; }
    .header .school { font-size: 13px; font-weight: 700; color: #e11d48; margin-bottom: 4px; }
    .header h1 { font-size: 17px; font-weight: 800; color: #0f172a; margin-bottom: 6px; }
    .header .meta { font-size: 9.5px; color: #64748b; }
    .filter-summary { margin-bottom: 10px; font-size: 10px; font-style: italic; color: #475569; }
    table { width: 100%; border-collapse: collapse; margin-top: 10px; }
    th { background: #e11d48; color: #fff; padding: 7px 8px; text-align: left; font-size: 9.5px; font-weight: 700; text-transform: uppercase; }
    td { padding: 6px 8px; border-bottom: 1px solid #f1f5f9; vertical-align: middle; }
    tr.even td { background: #fff1f2; }
    .totals { margin-top: 14px; text-align: right; font-size: 11px; }
    .totals strong { color: #e11d48; font-size: 13px; }
...
<body>
  <div class="header">
    ...
    <h1>${title}</h1>
    <div class="meta">Generated: ${now} &nbsp;|&nbsp; Total Debtors: ${rows.length}</div>
  </div>
  ${filterSummary ? `<div class="filter-summary">Filters Applied: ${filterSummary}</div>` : ''}
  <table>
    <thead><tr>${headers.map(h => `<th>${h}</th>`).join('')}</tr></thead>
    <tbody>${bodyRows}</tbody>
  </table>
  <div class="totals">Total Outstanding Debt: <strong>${fmtMoney(totalDebt)}</strong></div>
  <script>window.onload = function() { window.print(); };</script>
</body>
</html>`;
}

export default function DebtorsExporter({ schoolName, filterSummary, getExportRows }: Props) {
  ...
  const resolvedTitle = title.trim() || `Outstanding Balances Ledger – ${new Date().toLocaleDateString('en-NG')}`;
  ...
  const handleExcel = async () => {
    ...
    try {
      ...
      const sheetRows = rows.map((r) => ({
        ...
        'Debtor Name': r.name,
        'Type': r.type,
        'Contact / Class': r.contactOrClass,
        'Total Outstanding (NGN)': parseFloat(String(r.totalOwed)) || 0,
      }));

      const ws = XLSX.utils.json_to_sheet(sheetRows);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Debtors');
      XLSX.writeFile(wb, `debtors_ledger_${new Date().toISOString().slice(0, 10)}.xlsx`);
  ...
  return (
    <>
      ...
      <button onClick={() => { setTitle(''); setOpen(true); }} className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-700 text-sm font-semibold rounded-xl hover:bg-slate-50 transition-all shadow-sm">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                <Download className="h-5 w-5 text-rose-600" /> Export Debtors Ledger
              </h3>
            ...
            <div className="p-6 space-y-5">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 text-sm border border-slate-200 rounded-xl focus:ring-2 focus:ring-rose-500 outline-none"
                />
              </div>
              <div className="flex items-center gap-3 justify-end pt-2 border-t border-slate-100">
                ...
                <button onClick={handlePDF} disabled={exporting} className="inline-flex items-center gap-2 px-5 py-2.5 bg-rose-600 text-white text-sm font-semibold rounded-xl hover:bg-rose-700 disabled:opacity-50">
