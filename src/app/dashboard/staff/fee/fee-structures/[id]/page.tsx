...
export default function FeeStructurePricingPage() {
  ...
  if (loading) return (
    <div className="min-h-[500px] flex flex-col items-center justify-center gap-4">
      ...
      <p className="text-slate-400 font-bold tracking-widest text-xs uppercase">Loading Pricing Matrix...</p>
    </div>
  );

  if (!structure || !feeBlueprint || !activeSession) return (
    <div className="max-w-2xl mx-auto p-16 text-center bg-white rounded-2xl border border-slate-100 shadow-sm">
       ...
       <button onClick={() => router.push('/dashboard/staff/fee/fee-structures')} className="mt-8 px-6 py-2.5 bg-slate-900 text-white rounded-xl font-bold text-xs">Return to Fee Master</button>
    </div>
  );
    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-16 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        <div className="flex items-center gap-4">
          <button onClick={() => router.push('/dashboard/staff/fee/fee-structures')} className="p-2.5 text-slate-400 hover:text-slate-800 hover:bg-slate-50 border border-transparent rounded-xl transition-all">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold text-slate-900">{feeBlueprint.name}</h1>
              <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md ${occ === 'periodic' ? 'bg-blue-50 text-blue-700' : 'bg-amber-50 text-amber-700'}`}>
                {occ?.replace('_', ' ')}
              </span>
            ...
            <p className="text-xs text-slate-500 font-medium mt-0.5">Pricing for the {activeSession.start_year}/{activeSession.end_year} session</p>
          </div>
        </div>
        ...
        {canManage && (
          editing ? (
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <button onClick={cancelEditing} disabled={isSubmitting} className="flex-1 sm:flex-none px-5 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-50 shadow-sm transition-colors disabled:opacity-50">
                Cancel
              </button>
              <button onClick={handleSave} disabled={isSubmitting || hasNegative} className="flex-1 sm:flex-none px-6 py-2.5 bg-blue-600 text-white text-sm font-bold rounded-xl shadow-md shadow-blue-200 hover:bg-blue-700 transition-colors flex items-center justify-center gap-2 disabled:opacity-50">
                {isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
                {isNew ? 'Finalize & Activate' : 'Save Changes'}
              </button>
            </div>
          ) : (
            <button onClick={startEditing} className="flex items-center gap-2 px-5 py-2.5 bg-slate-900 text-white text-sm font-bold rounded-xl hover:bg-slate-800 transition-all shadow-sm">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <p className="text-sm font-extrabold text-slate-900">{feeBlueprint.name}</p>
              ...
              <div>
                ...
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-slate-100 text-slate-700 text-[10px] font-bold uppercase rounded-lg border border-slate-200">
                  <FolderOpen className="h-3 w-3 text-slate-500" /> {group?.name || 'Unknown'}
                </span>
              </div>
              <div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div className="p-6 space-y-6">
              {displayedPeriods.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {displayedPeriods.map(p => {
                    ...
                    return (
                      <div key={p.id} className={`p-4 rounded-xl border transition-all ${hasPrice ? 'border-slate-200 bg-white' : editing ? 'border-slate-200 bg-white' : 'border-dashed border-slate-200 bg-slate-50/50'}`}>
                        <div className="flex items-center justify-between mb-3">
                          ...
                          {p.is_current && (
                            <span className="px-2 py-0.5 bg-indigo-600 rounded-md text-[9px] font-bold text-white uppercase tracking-wide">Current</span>
                          )}
                        </div>
                        ...
                        {editing ? (
                          <div className="relative">
                            ...
                            <input
                              ...
                              placeholder="0.00"
                              className={`w-full pl-8 pr-3 py-2.5 border rounded-xl text-sm font-bold text-right focus:outline-none focus:ring-2 transition-all ${isNegative ? 'border-rose-300 focus:ring-rose-200 text-rose-600' : 'border-slate-200 focus:ring-blue-500 text-slate-800'}`}
                            />
                          </div>
                        ) : (
                          <p className={`text-lg font-black text-right ${hasPrice ? 'text-slate-900' : 'text-slate-300 italic text-sm font-medium'}`}>
              ...
              {/* Total Summary */}
              <div className="bg-slate-900 rounded-2xl p-6 flex items-center justify-between shadow-md">
                ...
                <p className="text-xl font-black text-white">{fmtMoney(total)}</p>
              </div>
            </div>
          </div>
        </div>
